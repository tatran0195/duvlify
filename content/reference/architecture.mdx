...
A page declares *what it is* in its frontmatter: its title, description, and
icon. `docs.config.ts` declares *where it sits*. `src/lib/navigation.ts`
...
`noindex` is a different switch. That page *is* published, routed, and
