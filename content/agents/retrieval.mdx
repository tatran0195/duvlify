...
purpose. It is *not* the tuning knob. Under reciprocal rank fusion its scale
...
- If more than a third of the index looks stale, the script deletes nothing
  ...
  edit.
- A failure to *list* is reported but does not fail the run. The upload half
...
fields, and they attach to the uploaded *item*, the whole page, not to its
...
<Callout title="Not the API's own checksum field" tone="note">
  ...
  literal string `"0"`, exactly the items that run had *created* rather than
